...
You can add your personal number (for example, your mobile phone) to place calls from the app.

**Outbound-only:** Personal numbers can **make outbound calls** but **cannot receive inbound calls** in the app.
...
### Billing Rules for Outbound Calls

- **Default billing:** Outbound calls are billed **per second**.
- **Short-call rounding rule:** If **50% or more of your last 100 outbound calls made in the app** have a duration **under 50 seconds**, then **starting from that point forward** we will bill the **first 60 seconds** of each outbound call (or any fraction of it) as **one full minute** (i.e., **rounded up to 1 minute**).
